...
export const actions = {
  deploy: async ({ request }) => {
    ...
    if( !data.get("bounty") ) return  { deployed: false, txid: "please enter the bounty amount" };


    if( !data.get("square") ) return  { deployed: false, txid: "please enter the square" };
  ...
  unlock: async ({ request }) => {
    ...
    if( !data.get("root") ) return  { success: false, txid: "Please enter the root.!" };
